        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                if stage == 1:
                    # 最後のチャンクのテスト結果を取得
                    cur.execute('''
                        SELECT cp.chunk_number,
                               (SELECT COUNT(*) FROM study_log sl
                                WHERE sl.user_id = %s AND sl.stage = 1 AND sl.mode = 'test'
                                AND sl.card_id IN (
                                    SELECT card_id FROM chunk_cards 
                                    WHERE user_id = %s AND source = %s AND stage = 1 
                                    AND page_range = %s AND difficulty = %s AND chunk_number = cp.chunk_number
                                )) as test_count,
                               (SELECT COUNT(*) FROM study_log sl
                                WHERE sl.user_id = %s AND sl.stage = 1 AND sl.mode = 'practice'
                                AND sl.card_id IN (
                                    SELECT card_id FROM chunk_cards 
                                    WHERE user_id = %s AND source = %s AND stage = 1 
                                    AND page_range = %s AND difficulty = %s AND chunk_number = cp.chunk_number
                        ...
                        LIMIT 1
                    ''', (user_id, user_id, source, page_range, difficulty,
                          user_id, user_id, source, page_range, difficulty,
                          user_id, user_id, source, page_range, difficulty,
                    ...
                    if last_chunk[1] > 0 and last_chunk[2] > 0:
                        return True
                ...
                cur.execute('''
                    SELECT COUNT(*) FROM (
                        ...
                        AND EXISTS (
                            ...
                            WHERE sl.user_id = %s AND sl.stage = %s AND sl.mode = 'test'
                            AND sl.card_id IN (
                                SELECT card_id FROM chunk_cards 
                        ...
                        AND (
                            EXISTS (
                                SELECT 1 FROM study_log sl
                                WHERE sl.user_id = %s AND sl.stage = %s AND sl.mode = 'practice'
                                AND sl.card_id IN (
                                    SELECT card_id FROM chunk_cards 
                                    WHERE user_id = %s AND source = %s AND stage = %s 
                                    AND page_range = %s AND difficulty = %s AND chunk_number = cp.chunk_number
                ...
                ''', (user_id, source, stage, page_range, difficulty,
                      user_id, stage, user_id, source, stage, page_range, difficulty,
                      user_id, stage, user_id, source, stage, page_range, difficulty,
                      user_id, stage, user_id, source, stage, page_range, difficulty))
...
def log_result():
    try:
        ...
        stage = session.get('stage', 1)
        session_mode = session.get('mode', 'test')
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute('''
                    INSERT INTO study_log (user_id, card_id, stage, mode, result)
                    VALUES (%s, %s, %s, %s, %s)
                ''', (user_id, card_id, stage, session_mode, result))
                ...
                if stage == 1:
                    ...
                    if session_mode == 'test':
                        ...
                        cur.execute('''
                            ...
                            FROM latest_results
                            WHERE rn = 1
                        ''', (user_id, source, stage, page_range, difficulty, current_chunk,
                              user_id, stage))
                        ...
                        cur.execute('''
                            UPDATE chunk_progress
                            SET test_completed = %s,
                                completed = CASE 
                                    WHEN %s = true AND chunk_number = (
                        ...
                        if known_cards < total_cards:
                            cur.execute('''
                                UPDATE chunk_progress
                                SET practice_needed = true
                                WHERE user_id = %s AND source = %s AND stage = %s 
                                AND page_range = %s AND difficulty = %s AND chunk_number = %s
                            ''', (user_id, source, stage, page_range, difficulty, current_chunk))
                    
                    # 練習モードの場合
                    elif session_mode in ['chunk_practice', 'practice'] and practicing_chunk:
                        # チャンク内の全カードの練習結果を確認
                        cur.execute('''
                            WITH chunk_cards AS (
                                SELECT c.id
                    ...
                    # 練習モードの場合
                    elif session_mode == 'practice' and practicing_chunk:
                        # チャンク内の全カードの練習結果を確認
                        cur.execute('''
                            WITH chunk_cards AS (
                                SELECT c.id
