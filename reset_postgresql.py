#!/usr/bin/env python3
"""
PostgreSQLデータベースをリセットして再作成するスクリプト
"""
...
def reset_postgresql():
    ...
    try:
        ...
        print("🔄 PostgreSQLデータベースをリセット中...")
        
        # 既存のテーブルを削除
        tables_to_drop = [
            'assignment_details', 'textbook_assignments', 'study_logs', 
            'study_sessions', 'questions', 'units', 'textbooks',
            'choice_study_log', 'choice_questions', 'choice_units', 
            'choice_textbooks', 'input_study_log', 'input_questions', 
            'input_units', 'input_textbooks', 'user_settings', 'image', 
            'chunk_progress', 'study_log', 'users'
        ]
        ...
        for table_name in tables_to_drop:
            try:
                ...
                print(f"⚠️  テーブル {table_name} の削除でエラー: {e}")
        ...
        # インデックス作成
        indexes = [
            "CREATE INDEX idx_study_log_user_stage_mode ON study_log(user_id, stage, mode);",
            "CREATE INDEX idx_study_log_composite ON study_log(user_id, stage, mode, card_id, id DESC);",
            "CREATE INDEX idx_image_source_page ON image(source, page_number);",
            "CREATE INDEX idx_image_source_level ON image(source, level);",
            "CREATE INDEX idx_chunk_progress_user_source_stage ON chunk_progress(user_id, source, stage);",
            "CREATE INDEX idx_study_log_card_result ON study_log(card_id, result, id DESC);",
            "CREATE INDEX idx_user_settings_user_source ON user_settings(user_id, source);",
            "CREATE INDEX idx_questions_textbook_unit ON input_questions(textbook_id, unit_id);",
            "CREATE INDEX idx_choice_units_textbook ON choice_units(textbook_id, unit_number);",
            "CREATE INDEX idx_choice_questions_unit ON choice_questions(unit_id);",
            "CREATE INDEX idx_choice_study_log_user_question ON choice_study_log(user_id, question_id);",
            "CREATE INDEX idx_choice_study_log_user ON choice_study_log(user_id, answered_at);"
        ]
